...
const MOTOR_SPEED_MIN_DELAY_MS = 1200;
const MOTOR_SPEED_MAX_DELAY_MS = 180;

function motorStepToDelayMs(step) {
  const s = Math.max(1, Math.min(10, Math.round(Number(step) || 5)));
  const ratio = (s - 1) / 9;
  return Math.round(MOTOR_SPEED_MIN_DELAY_MS - ratio * (MOTOR_SPEED_MIN_DELAY_MS - MOTOR_SPEED_MAX_DELAY_MS));
}

function motorDelayMsToStep(delayMs) {
  const d = Math.max(MOTOR_SPEED_MAX_DELAY_MS, Math.min(MOTOR_SPEED_MIN_DELAY_MS, Number(delayMs) || 600));
  const ratio = (MOTOR_SPEED_MIN_DELAY_MS - d) / (MOTOR_SPEED_MIN_DELAY_MS - MOTOR_SPEED_MAX_DELAY_MS);
  return Math.max(1, Math.min(10, Math.round(1 + ratio * 9)));
}

function updateMotorSpeedLabel(step) {
  const s = Math.max(1, Math.min(10, Math.round(Number(step) || 5)));
